# package.ps1 — Build and package the Playlists Plugin for Vido
# Produces: vido-playlists-1.0.0.zip
...
$pluginId   = 'com.vido.playlists'
$projectDir = Join-Path $PSScriptRoot 'src\PlaylistPlugin'
$publishDir = Join-Path $PSScriptRoot 'publish'
$stageDir   = Join-Path $PSScriptRoot "stage\$pluginId"
$zipName    = "vido-playlists-$version.zip"
$zipPath    = Join-Path $PSScriptRoot $zipName
$deployDir  = Join-Path $env:APPDATA "Vido\plugins\$pluginId"

Write-Host "=== Building Playlists Plugin v$version ===" -ForegroundColor Cyan
...
dotnet publish "$projectDir\PlaylistPlugin.csproj" -c Release -o $publishDir --nologo -v quiet
